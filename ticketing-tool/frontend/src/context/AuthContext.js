...
export function AuthProvider({ children }) {
  ...
  const login = async (email, password) => {
    ...
    try {
      const response = await fetch("http://3.135.229.169:8000/auth/login", {
        method: "POST",
        headers: {
  ...
  const register = async (email, password, fullName) => {
    ...
    try {
      const response = await fetch("http://3.135.229.169:8000/auth/register", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
        },
  ...
  const refreshUserData = async () => {
    ...
    try {
      ...
      const response = await fetch(`http://3.135.229.169:8000/user-profiles/email/${encodeURIComponent(user.email)}`);
      ...
      if (response.ok) {
        ...
        const usersResponse = await fetch(`http://3.135.229.169:8000/auth/login`, {
          method: "POST",
