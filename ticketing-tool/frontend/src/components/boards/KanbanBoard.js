...
const listIssues = async (projectId) => {
  try {
    const response = await fetch('http://3.135.229.169:8000/tickets');
    if (response.ok) {
      ...
      if (projectId) {
        ...
        const epicsResponse = await fetch(`http://3.135.229.169:8000/epics?project_id=${projectId}`);
        if (epicsResponse.ok) {
...
const listProjects = async (userEmail = null) => {
  try {
    const url = userEmail 
      ? `http://3.135.229.169:8000/projects?user_email=${encodeURIComponent(userEmail)}`
      : 'http://3.135.229.169:8000/projects';
...
const listEpics = async (projectId) => { 
  try {
    const url = projectId ? `http://3.135.229.169:8000/epics?project_id=${projectId}` : 'http://3.135.229.169:8000/epics';
...
const deleteEpicAPI = async (epicId) => {
  try {
    ...
    const response = await fetch(`http://3.135.229.169:8000/epics/${numericId}`, {
      method: 'DELETE',
    });
    if (response.ok) {
      return true;
...
const moveIssue = async (issueId, status) => {
  try {
    ...
    const response = await fetch(`http://3.135.229.169:8000/tickets/${issueId}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
...
const updateIssueAPI = async (updatedIssue) => { 
  try {
    ...
    const response = await fetch(`http://3.135.229.169:8000/tickets/${updatedIssue.id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
