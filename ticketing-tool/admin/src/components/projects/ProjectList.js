...
const listProjects = async () => {
  try {
    const response = await fetch('http://3.135.229.169:8000/projects');
    if (response.ok) {
...
const updateProject = async (id, project) => {
  try {
    ...
    const response = await fetch(`http://3.135.229.169:8000/projects/${id}`, {
      method: 'PUT',
      headers: {
        'Content-Type': 'application/json',
      },
...
export default function ProjectList() {
  ...
  const loadRegisteredUsers = async () => {
    try {
      const response = await fetch('http://3.135.229.169:8000/users-management');
      if (response.ok) {
