...
const listIssues = async (projectId) => {
  try {
    ...
    const url = projectId 
      ? `http://3.135.229.169:8000/admin/tickets?project_id=${projectId}`
      : 'http://3.135.229.169:8000/admin/tickets';
...
const listEpics = async (projectId = null) => { 
  try {
    ...
    const url = projectId 
      ? `http://3.135.229.169:8000/admin/epics?project_id=${projectId}`
      : 'http://3.135.229.169:8000/admin/epics';
...
const createEpicAPI = async (epicName, projectId, projectTitle, userName) => {
  try {
    ...
    const url = new URL('http://3.135.229.169:8000/epics');
    if (userName) {
...
const deleteEpicAPI = async (epicId) => {
  try {
    ...
    const adminEpicsResponse = await fetch(`http://3.135.229.169:8000/admin/epics?project_id=`);
    if (adminEpicsResponse.ok) {
      ...
      if (adminEpic) {
        await fetch(`http://3.135.229.169:8000/admin/epics/${adminEpic.admin_epic_id}`, {
          method: 'DELETE'
        });
      }
    }
    ...
    await fetch(`http://3.135.229.169:8000/epics/${numericEpicId}`, {
      method: 'DELETE'
    });
...
const moveIssue = async (issueId, status) => {
  try {
    ...
    const response = await fetch(`http://3.135.229.169:8000/tickets/${numericTicketId}`, {
      method: 'PUT',
      headers: { 'Content-Type': 'application/json' },
  ...
  try {
    ...
    try {
      ...
      if (userEmail) {
        ...
        const usersResponse = await fetch(`http://3.135.229.169:8000/users-by-email/${encodeURIComponent(userEmail)}`);
        ...
        if (usersResponse.ok) {
          ...
          const createUserResponse = await fetch('http://3.135.229.169:8000/users-simple', {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
              email: userEmail,
    ...
    const url = new URL('http://3.135.229.169:8000/tickets');
    if (epicId) {
...
const deleteIssueAPI = async (issueId) => {
  try {
    ...
    const adminTicketsResponse = await fetch(`http://3.135.229.169:8000/admin/tickets`);
    if (adminTicketsResponse.ok) {
      ...
      if (adminTicket) {
        await fetch(`http://3.135.229.169:8000/admin/tickets/${adminTicket.admin_ticket_id}`, {
          method: 'DELETE'
        });
      }
    }
    ...
    await fetch(`http://3.135.229.169:8000/tickets/${numericTicketId}`, {
      method: 'DELETE'
    });
...
export default function KanbanBoard() {
  ...
  const loadAllData = async () => {
    try {
      ...
      const projectsResponse = await fetch('http://3.135.229.169:8000/projects');
